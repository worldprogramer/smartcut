...
4. 配音，软件支持[微软Azure](https://azure.microsoft.com/zh-cn/products/ai-services/ai-speech/)和[抖音火山引擎](https://www.volcengine.com/product/voice-tech)的配音服务，注册账号和创建应用，你可以获取对应的sk、access token等信息,你也可以加载从其他地方生成好的配音文件和字幕文件。
5. 字幕，软件支持自动识别字幕文件，当你选择微软配音时，可以使用本地字幕，选择一个文件夹，用来保存识别字幕的模型文件，他首次会自动下载所需的模型，他会自动完成字幕生成，使用火山引擎或加载自己的字幕不需要配置。

#### 2. 创建项目
你可以创建一个项目，并重名名它，当你创建一个项目时，他会在你的保存文件夹下创建一个项目文件夹。
#### 3. 视频生成
1. 在确认所有的设置设置完成之后（通常你只需要在首次使用时设置），你可以开始生成视频了。
2. 在生成视频之前，你需要导入自己的视频素材。注意在同一个项目你导入的素材应该具有相同分辨率的比例（比如不要把16：9  4：3 素材混在一起），你可以提前在素材分割功能里预处理好素材，他也支持批量处理。
3. 素材可以支持按镜头场景导入或者或全部导入到一个镜头。当按镜头导入时，视频会按照镜头顺序依次从每个镜头选择素材（通常他被用在音画同步的场景下），最后一个镜头可以设置为通用素材，因为如果素材不够用，会从最后一个镜头中继续尝试获取素材。更简单的时你可以把所有素材导入到第一个镜头中，并选择全部模式，他会进行自动混剪。
4. 你可以添加背景音乐，生成视频时会自动为你加上背景音乐，注意在设置中调整背景音乐的音量。
5. 在设置好后（通常只需要首次设置）添加你的文案(可以一次选多个)，点击开始生成，软件会开始自动生成视频。
6. 生成完成后可以鼠标右键打开视频的位置，在视频所在的文件夹中也可以找到对应音频、字幕、视频文件，如果需要重新生成视频或者音频，需要删除对应的视频文件或音频及其字幕文件。
